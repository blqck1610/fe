...
const CartItem = ({ cartItem }) => {
  ...
  useEffect(() => {
    // handleEditQuantity(0);
  }, []);
  // const handleEditQuantity = async(quantity) => {

  // }
  const handleEditQuantityPlus = () =>{
    ...
    <CartTableRowWrapper key={cartItem.id}>
      ...
      <td>
        <div className="cart-tbl-qty flex items-center">
          <button className="qty-dec-btn" onClick={() => handleEditQuantity("-1")}>
            <i className="bi bi-dash-lg"></i>
          </button>
          <span className="qty-value inline-flex items-center justify-center font-medium text-outerspace">
            {cartItem.quantity}
          </span>
          <button className="qty-inc-btn" onClick={() => handleEditQuantity(1)}>
