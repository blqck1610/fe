...
const CartItem = ({ cartItem , setRender}) => {
  ...
  const handleEditQuantity = async(productId, value) =>{
      if(value === 1){
        console.log(1);
        const await CartService.updateCart(productId, quantity);
        setRender(true);
    ...
    <CartTableRowWrapper key={cartItem.id}>
      <td>
        <div className="cart-tbl-prod grid">
          <div className="cart-prod-img">
            <img src={cartItem.product.imageUrl} className="object-fit-cover" alt="" />
          </div>
      ...
      <td>
        <div className="cart-tbl-qty flex items-center">
          <button className="qty-dec-btn" onClick={() => handleEditQuantity(cartItem.product.productId,-1)}>
            <i className="bi bi-dash-lg"></i>
          </button>
          <span className="qty-value inline-flex items-center justify-center font-medium text-outerspace">
            {cartItem.quantity}
          </span>
          <button className="qty-inc-btn" onClick={() => handleEditQuantity(cartItem.product.productId,1)}>
