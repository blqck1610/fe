...
const ProductDetailsScreen = (props) => {
  ...
  // const [keyForRerender, setKeyForRerender] = useState(0); // State to trigger re-render

  useEffect(() => {
    fetchAllData(productId);
  }, [productId]); // Add keyForRerender to dependencies array
  const fetchAllData = async (id) => {
    try {
      const [productRes, reviewsRes] = await Promise.all([
        ProductService.fetchProductById(id),
        ProductService.fetchReviewsByProductId(id)
      ...
      if (productRes) {
        ...
        console.log("No content in product response");
      }
      if (reviewsRes && reviewsRes.content) {
        console.log("reviews:", reviewsRes);
        setreviews(reviewsRes);
      } else {
        console.log("No content in reviews response");
      }
      } catch (error) {
  ...
  //   setKeyForRerender(prevKey => prevKey + 1); // Increment key to trigger re-render
  // };
  ...
  if(reviews && reviews.totalElements > 0) {
    ...
    for(let i = 0; i < reviews.length; i++) {
        temp += reviews.content[i].rating;
    }
    setRating(temp/reviews.totalElements);
  }
  ...
  const breadcrumbItems = [
    { label: "Shop", link: "/" },
    { label: product.brandName, link: `products/${product.brandName}` },
    { label: product.productName, link: "" },
  ];
  ...
  const handleAddToCart = async() => {
      if(!UserService.isAuth()){
        ...
        const res = await ProductService.addProductToCart(UserService.getUsername(), productId);
        toastr.success(res
          ...
          <ProductDetailsWrapper>
            ...
            <div className="flex items-center rating-and-comments flex-wrap">
              ...
              <div className="prod-comments flex items-start">
                ...
                <span className="prod-comment-text text-sm text-gray">
                  {reviews.totalElements} comment(s)
                </span>
              </div>
            </div>
            ...
            <div className="btn-and-price flex items-center flex-wrap">
              ...
              >
                ...
                <span className="prod-add-btn-text" onClick = {()=>handleAddToCart()}>Add to cart</span>
              </BaseLinkGreen>
