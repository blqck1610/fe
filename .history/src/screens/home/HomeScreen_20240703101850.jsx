...
const HomeScreen = () => {
  ...
  const [menCatalog, setMensCatalog] = useState([]);
  const [womenCatalog, setWomensCatalog] = useState([]); //
  ...
  const getMenCatalog = async () => {
    let res = await fetchMenProducts();
    if(res && res.content){
      setMensCatalog(res.content);
      console.log("men products: "+ menCatalog
    }
